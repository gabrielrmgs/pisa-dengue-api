...
/**
 * Endpoints de mapa consumidos diretamente pelo Leaflet no frontend.
 *
 * Rotas:
 *   GET /api/v1/mapa/{geocodigo}/geojson          → GeoJSON bruto (sem dados de casos)
 *   GET /api/v1/mapa/{geocodigo}/geojson/heatmap   → GeoJSON enriquecido com casos por bairro
 *   GET /api/v1/mapa/{geocodigo}/bairros            → Lista de bairros (para filtros)
 */
@Path("/api/v1/mapa/{geocodigo}")
...
public class MapaResource {
    ...
    @Inject BairroGeoJsonRepository geoJsonRepo;
    ...
    public Response getGeoJson(@PathParam("geocodigo") String geocodigo) {
        String json = geoJsonRepo.getGeoJsonBruto(geocodigo);
        return Response.ok(json)
                .header("Cache-Control", "public, max-age=86400") // 24h — geometria não muda
                .build();
    }
    ...
    // @Produces("application/geo+json")
    // @Operation(summary = "GeoJSON com casos por bairro injetados nas properties — usado pelo Leaflet choropleth")
    ...
    //     Map<String, Integer> casosPorBairro = casosPorBairroService.getCasosPorBairro(geocodigo, ano, mes);
